...
PermissionRules.prototype.setConfig = function (options) {
    ...
    _.keys(self.config).forEach(function (key) {
        if (options.hasOwnProperty(key)) self.config[key] = options[key];
    });
};
...
PermissionRules.prototype.permissionDenied = function (req, res, next) {
    ...
    if (self.config.redirectUrl && req.originalUrl != self.config.redirectUrl)
        return res.redirect(self.config.redirectUrl);
...
PermissionRules.prototype.middleware = function () {
    ...
    return function (req, res, next) {
        ...
        function authenticate(rule, callback) {
            ...
            if ((!rule.users || (rule.users.length == 0 || _.contains(rule.users, '*') || _.contains(rule.users, '?')))
